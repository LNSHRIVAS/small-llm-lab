...
Given three (or more) model sizes trained on the **same corpus and stack**, with public training-loss curves, we can estimate E_true by fitting E_app across N - **zero new training**.
...
**Protocol:** late pseudo-epoch **C\*** as E_app (steps ≥ 50,000); α = 0.34; **6-point overdetermined OLS** (4 DOF). Uncertainty = LOO std - not in-sample RMSE.
...
**Reference:** Chinchilla paper reports E ≈ 1.69 nats on **MassiveText** (different corpus) - delta ≈ −0.40 nats vs our Pile estimate; not a pass/fail comparison.
...
| Kempner proof-pile / starcoder | 13 | ~1.26-1.75 | ✗ | non-monotonic E_app(N) |
| OPT @ 10B tokens | 6 | 3.22 ± 0.05 | ✗ | 6.7B→13B bump |
| OWT (flat E_app) | 3 | 2.49 | ✗* | reference only |

\* OWT uses published flat E_app, not epoch curves.
...
| Test | Result |
|------|--------|
| Universal single E_true | **REJECTED** - spread 1.52 nats across 13 passes |
...
**Chinchilla original 400-run grid:** MassiveText and training code are proprietary. Individual run logs are not public. We only compare qualitatively to their published E ≈ 1.69 nats on MassiveText.
...
3. **Multi-corpus success is a method finding, not a universal law.** The **procedure** (fixed α, holdout, LOO) transfers; **E_true** values do not collapse to one number. Failures on truncated or non-monotonic ladders are expected and useful.
