...
CE(N,t) = C_inf(N) + (H - C_inf(N)) * (1 + t/tau(N))^(-p)
...
- `C_inf(N) = E_true + k * N^(-alpha)` with **alpha = 0.34** fixed; fit E_true and per-size tau (and k if needed)

**Compare to baselines:**

- Free p_N per size (same functional form)
- Triangulation-only E_true ≈ 2.485 nats from Act IV-A (independent anchor)
...
4. **Floor recovery:** under fixed shared p, regress recovered C_inf(N) to E_true via N^(-0.34) law.
...
| **S2** | Floor recovery | Recovered E_true under shared p in **[2.39, 2.58]** nats (triangulation band from Act IV-A ±0.10) |
...
| ID | Gate | Threshold |
|----|------|-----------|
| **F1** | Floor drift under shared p | |E_true_recovered - 2.485| **> 0.10** nats **and** S1 still passes (shape looks universal but floor won't lock) |
...
- Fixed-p floor regression: E_true → **2.744** nats vs triangulation **2.485** → **S2 fails** (+0.26 nats)
