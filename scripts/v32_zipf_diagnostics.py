# =============================================================================
# v32 — Zipf-Geometry Diagnostic Suite (clean, controlled)
# =============================================================================
#
# Two run modes selected via the MODE constant below:
#
#   MODE = 'main'          — Full v31-style 2-epoch training with H1..H8
#                            diagnostics. ~8-9 h on a T4. Tests H1..H8
#                            simultaneously in one run.
#
#   MODE = 'scaling_scan'  — Multi-variant short trainings to test the
#                            A_floor ∝ V² / T law (5 variants, ~2-3 h on T4).
#                            Use a SEPARATE Colab session.
#
...
# =============================================================================
# HYPERPARAMETERS  (identical to v31 — do not change for comparability)
# =============================================================================
...
LOCAL_CACHE_DIR = _REPO_ROOT / 'data' / 'cache' / 'wikitext103_gpt2'
...
RUN_NAME = 'v32_zipf_phase_transition'

# =============================================================================
# MODE DISPATCH
# =============================================================================
#
# 'main'          → full v31-style 2-epoch training with H1..H8 diagnostics.
#                   Time budget: same as v31 (~8-9h on T4). Tests H1..H8.
#
...
DIAG_FREQ        = 200     # opt steps between H3/H5/H6 grad-diagnostics
SAVE_CHECKPOINTS = True    # save per-epoch checkpoints for post-hoc analysis
# Phase-transition continuation: load vMF ep2 only (not v32_standard_softmax_*).
# Filename may be legacy (below) or {RUN_NAME}_ep2.pt — set RESUME_FROM to whatever
# exists on disk; new saves use RUN_NAME in the checkpoint filename.
RESUME_FROM = ARTIFACT_DIR / 'v32_zipf_full_diagnostics_ep2.pt'
...
#
# Each variant trains a fresh model on train_tokens[:T_corpus] with:
#   • V_eff-way LM head (top types by frequency on that subset) — real V-way
#     vMF/softmax normalizer, not full-V logits + CE masking.
#   • T_trained = opt_steps × SCAN_BATCH × SCAN_ACCUM × SEQ_LEN  scales with
#     T_corpus via SCAN_TOKEN_PASS_FRAC (same expected pass fraction of subset).
# Per variant we fit  CE(t) = A/t + C*  and report A.
#
# Regression: log(A) vs log(V_eff² / T_trained).  Slope ≈ 1.0 supports A ∝ V²/T.
...
SCAN_TOKEN_PASS_FRAC   = 0.28   # target fraction of T_corpus as training token-slots
...
class TransformerBlock(nn.Module):
    ...
    def attention(self, x, is_causal=True):
        b, t, d = x.shape
    ...
    def forward(self, x, is_causal=True):
        x = x + self.dropout(self.attention(self.norm_attn(x), is_causal=is_causal))
...
class BreakthroughMicroTransformerE4Scan(nn.Module):
    """
    Scaling-scan LM: full input embedding, V_eff-way output head over the
    V_eff most frequent types (on the training subset). Logits are true
    V_eff-dimensional vMF (or linear if HEAD_MODE=='linear'); CE ignores
    targets outside the head via global_to_local == -100.

    MTP is omitted so the scan matches a single next-token CE stream.
    """
    ...
    def forward(self, ids, labels=None, kappa_cap=None, class_weight=None,
                head_mode=HEAD_MODE):
        del kappa_cap, head_mode  # scan uses module HEAD_MODE; cap unused
        h = self.forward_features(ids)
...
def train_model(model, train_loader, val_loader, test_tokens,
    ...
    if resume_state is not None:
        ...
        # torch.load(..., map_location=cuda) moves every ckpt tensor to GPU;
        # H1/H4 diagnostics expect init snapshots on CPU next to .cpu() weights.
...
# =============================================================================
# SCALING SCAN  —  test  A_floor ∝ V² / T
# =============================================================================
#
# THEORY (under test, not assumed):
#
...
#     (V_eff-way vMF/linear logits; not full-V softmax + masking).
...
def _print_environment():
    sep = '═' * 78
    print('\n' + sep)
    print(f'  v32  ·  Zipf-geometry diagnostic suite  ·  MODE = {MODE!r}')
    print(sep)
    ...
    if MODE == 'main':
        print(f'  EPOCHS      : {EPOCHS}   '
              f'RESUME_FROM exists={RESUME_FROM.is_file()}')
        print(f'               {RESUME_FROM}')
...
def main():
    ...
    if MODE == 'main':
        ...
        if RESUME_FROM.exists():
            ...
            print(f'[resume] no checkpoint found at {RESUME_FROM}')
